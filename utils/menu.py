...
class InitSet:
    ...
    def __init__(self, mode):
        ...
        self.bilibili_default_init = {
            "save_folder": 'bilibili_download',
            "uid": "",
            "semaphore": 10,
            "single_page_num": 12,
            "start_page": None,
        ...
        choice = {
            "rank": self._init_rank_print,
            'tags': self._init_tags_print,
            'id': self._init_id_print,
            'bilibili': self._init_bilibili_print,
        }
        ...
        if juge in ['Y', 'y']:
            ...
            while True:
                ...
                if re == 1:
                    ...
                    self.rank_default_init['rank_mode'] = self.check.input(
                        "选择筛选方式(['今日','本周','本月','新人','受男性欢迎','受女性欢迎']:",
                        check_mode=CHECK_MODE_STR_LIST,
                        str_list=['今日', '本周', '本月', '新人', '受男性欢迎', '受女性欢迎'])
                elif re == 6:
                    self.rank_default_init['semaphore'] = self.check.input('设置最大同时下载数:',
                                                                           check_mode=CHECK_MODE_INT_RANGE,
                                                                           min_val=1)
    ...
    def _init_id_print(self):
        ...
        if juge in ['Y', 'y']:
            ...
            while True:
                re = self.check.input('选择需要更改的内容', check_mode=CHECK_MODE_INT_RANGE, min_val=1, max_val=4)
                if re == 1:
    ...
    def _init_bilibili_print(self):
        display_head("bilibili下载模式")

        print('1. 是否需要指定储存文件夹,默认文件夹为bilibili_download文件夹')
        print('2. 是否需要修改最大同时下载数量,默认为10')
        print('3. 是否需要修改单页显示数量,默认为12')
        print('4. 是否需要修改开始的页码,默认为第一页')
        print('5. 是否需要修改终止页码,默认为最后一页')
        juge = self.check.input('\n使用默认方式请按Y,需要更改请按N\n输入:', check_mode=CHECK_MODE_STR_LIST,
                                str_list=['Y', 'y', 'N', 'n'])
        if juge in ['Y', 'y']:
            return None
            ...
            while True:
                re = self.check.input('选择需要更改的内容', check_mode=CHECK_MODE_INT_RANGE, min_val=1, max_val=5)
                if re == 1:
                    self.bilibili_default_init['save_folder'] = input('输入储存文件夹:')
                elif re == 2:
                    self.bilibili_default_init['semaphore'] = self.check.input('设置最大同时下载数:',
                                                                               check_mode=CHECK_MODE_INT_RANGE,
                                                                               min_val=1)
                elif re == 3:
                    self.bilibili_default_init['single_page_num'] = self.check.input('设置单页显示数量:',
                                                                                     check_mode=CHECK_MODE_INT_RANGE,
                                                                                     min_val=1)
                elif re == 4:
                    self.bilibili_default_init['start_page'] = self.check.input('设置终止的页码:',
                                                                                     check_mode=CHECK_MODE_INT_RANGE,
                                                                                     min_val=1)
                elif re == 5:
                    self.bilibili_default_init['end_page'] = self.check.input('设置终止的页码:',
                                                                                     check_mode=CHECK_MODE_INT_RANGE,
                                                                                     min_val=1)
                n = self.check.input('\n是否修改完毕?修改完毕按Y,继续修改按N\n输入:', check_mode=CHECK_MODE_STR_LIST,
                                     str_list=['Y', 'y', 'N', 'n'])
                ...
                if n in ['Y', 'y']:
                    return self.bilibili_default_init
...
class Menu:
    def __init__(self, modeset, mode=None):
        ...
        if mode == 'tags':
            self.tag = input("输入标签:")
            self.bookmarkCount = self._get_bookmark_count() if modeset.bookmarkCount else ''
            ...
            self.resolution = self._get_resolution() if modeset.resolution else ''
            self.ratio = self._get_ratio() if modeset.ratio else ''
            self.pageList = self._get_page_list() if modeset.page else ['&p=1']
            # self.dateList = self.__get_data_list() if modeset.DataTime else [
            #     datetime.datetime.today().strftime('%Y-%m-%d').replace('-', '')]

        elif mode == 'rank':
            self.pageList = self._get_page_list() if modeset.page else ['&p=1']
            self.dateList = self._get_data_list() if modeset.DateTime else "&date=" + (datetime.datetime.now() +
                                                                                       datetime.timedelta(
                                                                                           days=-2)).strftime(
                '%Y-%m-%d').replace('-', '')
